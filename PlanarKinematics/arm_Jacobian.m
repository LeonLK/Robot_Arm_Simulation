function [J,...
          ...
          R_links] = arm_Jacobian(link_vectors,joint_angles,joint_axes,link_number,prismatic)
...
%   R_links: The orientations of the individual links
    ...
    v = link_end_set_with_base{link_number+1};
    v_set = link_end_set_with_base;
    v_diff = vector_set_difference(v,v_set);
    ...
    for i = 1:numel(joint_axis_vectors_R)
    ...
    for i = 1:link_number
        if prismatic(i)
            J(1:3,i) = joint_axis_vectors_R{i};
            J(4:6,i) = [0;0;0];
        else
            J(1:3,i) = cross(joint_axis_vectors_R{i},v_diff{i});
            J(4:6,i) = joint_axis_vectors_R{i};
        end
    end
